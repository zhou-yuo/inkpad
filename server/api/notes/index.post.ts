...
export default defineEventHandler(async (event) => {
  ...
  await getDb(event).prepare(`
    INSERT INTO notes (id, user_id, encrypted_title, encrypted_body, title_iv, body_iv, category_id, tags, preview_hash, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, user.id, encryptedTitle, encryptedBody, titleIv, bodyIv, categoryId, JSON.stringify(tags), await sha256(encryptedTitle), timestamp, timestamp).run()

  return { note: { id, encryptedTitle, encryptedBody, titleIv, bodyIv, categoryId, tags, createdAt: timestamp, updatedAt: timestamp } }
})
